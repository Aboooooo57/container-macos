...
**Status:** ✅ A1–A6 implemented — `compose logs` (`-f`), `exec`, `stop`/`start`/`restart`, `pull`, `config`, and `container top` (runs `ps` in the container via the exec process path). Remaining: A7 `image import`, A8 Docker aliases.
